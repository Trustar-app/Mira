...
# 多模态意图识别与流程调度节点
def mira(state: MiraState) -> Command[Literal["user_profile_creation_subgraph", "user_profile_edit_subgraph", "skin_analysis_subgraph", "product_recommend_subgraph", "product_analysis_subgraph", "care_makeup_guide_subgraph"]]:
    """
    多模态意图识别与流程调度节点。
    """
...
def call_product_analysis_subgraph(state: MiraState):
    if state.get("resume", False):
        intent = recognize_intent_with_current_flow(state["multimodal_text"], state.get("current_flow"))
        if intent != state.get("current_flow"):
            return Command(goto=intent_to_subgraph[intent], update={"current_flow": intent, "resume": False})
    ...
    subgraph_output = product_analysis_graph.invoke(subgraph_input)
    return {
        "user_profile": subgraph_output.get("user_profile"),
        "products_directory": subgraph_output.get("products_directory"),
        "messages": subgraph_output.get("messages")[-1:],
